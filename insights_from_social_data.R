...
lapply(package_list, install.packages, character.only=T)
...
wordcloud(wc_data$handle[klout_filter], wc_data$Klout[klout_filter], scale=c(2,0.01), min.freq=20, max.words=200, random.order=FALSE, rot.per=.5, use.r.layout = TRUE)  

wordcloud(wc_data$handle[klout_filter], log10(wc_data$Followers[klout_filter]), scale=c(2,0.01), min.freq=20, max.words=200, random.order=FALSE, rot.per=.5, use.r.layout = TRUE, colors = brewer.pal(9,"BrBG")) 

idx <-order(-wc_data$Followers,-wc_data$Klout)
plot(wc_data$Followers[idx[1:20]],wc_data$Klout[idx[1:20]],log="x",xlab="Number of followers",ylab="Klout")
text(wc_data$Followers[idx[1:20]],wc_data$Klout[idx[1:20]], labels=wc_data$handle[idx[1:20]], cex= 0.7, pos=3)

# Exercise 2.3: Brand-Specific User Scoring
r <-POST("http://webservice-api.available.from.instructor", body=tweets_data, encode = "json")
stop_for_status(r)
